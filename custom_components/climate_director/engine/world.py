...
class WorldState:
    ...
    def preconditioning(self, zone_id: str) -> bool:
        ...
        Eén grens, en die hoort hier: de klok van het verzoek zelf (tot wanneer
        het loopt). Er is geen tijdvenster meer - een handmatig verzoek gaat
        altijd voor, ongeacht het uur. Alles wat aan een verzoek hangt leest
        deze ene definitie, dus een verlopen verzoek geeft nergens een
        uitzondering.

        One bound, and it belongs here: the request's own clock (until when it
        runs). There is no time window anymore - a hand-given request always
        outranks the automation, whatever the hour. Everything hanging off a
        request reads this one definition, so an expired request grants no
        exception anywhere.
        """
