...
from .families import MODE_FAN_ONLY, MODE_OFF, ModeFamily, preferred_mode
...
def _build_commands(
    ...
    return tuple(sorted(commands, key=_command_order))
...
def _command_order(command: UnitCommand) -> tuple[int, str]:
    """Return the ordering key that puts stops ahead of starts."""
    rank = {MODE_OFF: 0, MODE_FAN_ONLY: 1}.get(command.hvac_mode, 2)
